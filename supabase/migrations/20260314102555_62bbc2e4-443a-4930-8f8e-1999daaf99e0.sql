...
AS $$
  ...
  OR EXISTS (
    -- Check migrated Wix bookings by staff name match
    SELECT 1
    FROM public.staff s
    JOIN public.migrated_bookings mb ON lower(mb.staff_name) = lower(s.name)
    ...
    WHERE s.auth_user_id = auth.uid()
      ...
      AND mb.booking_date >= (CURRENT_DATE - INTERVAL '7 days')
  );
$$;
