...
serve(async (req) => {
  ...
  try {
    ...
    if (retryFailed && existingCampaignName) {
      ...
      // Delete old failed logs before retrying
      ...
      for (let i = 0; i < failedLogs.length; i += BATCH_SIZE) {
        ...
        for (const entry of batch) {
          const result = await sendOneSms(entry.phone, message, twilioUrl, twilioAuth);
    ...
    // Track who has upcoming bookings
    ...
    // Resume logic: skip already-sent phones for this campaign
    ...
    for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
      ...
      for (const recipient of batch) {
        const result = await sendOneSms(recipient.phone, message, twilioUrl, twilioAuth);
        const status = result.ok ? "sent" : "failed";
        await supabase.from("bulk_sms_log").insert({
